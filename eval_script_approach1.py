...
HP_FOLDER = "approach_2"

def eval_gp(gp_id):
    # Input data
    gp_eval_path = f"json_folder/okvqa_train_jsons/gp{gp_id}_val_cls_0.json"
    ...
    # Load model
    disable_torch_init()
    model_path = f'/home/ubuntu/workspace/MoE-LLaVA/output_trained/{HP_FOLDER}/MoE-LLaVA-StableLM-1.6B-4e-okvqa_gp{gp_id}'
    device = 'cuda'
    ...
    for i in tqdm(range(num_data)):
        ...
        if (i % 100 == 0 and i != 0) or i == num_data - 1:
            with open(f"okvqa_eval/{HP_FOLDER}/gp{gp_id}_outputs/saved_outputs_{save_file_id}.pkl", 'wb') as f:
    ...
    for i in range(num_files):
        with open(f"okvqa_eval/{HP_FOLDER}/gp{gp_id}_outputs/saved_outputs_{i}.pkl", "rb") as f:
            outputs_i = pickle.load(f)
            questions_to_outputs.update(outputs_i)
    output_path = f"okvqa_eval/{HP_FOLDER}/qid_to_output_gp_{gp_id}.pkl"
