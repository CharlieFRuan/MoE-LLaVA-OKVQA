...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer,
    ...
    print("CHARLIE TRAIN 3")

    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
        print("CHARLIE TRAIN 4")
        # Only save Adapter
    ...
    if trainer.deepspeed:
        print("CHARLIE TRAIN 5")
        torch.cuda.synchronize()
        trainer.save_model(output_dir)
        return

    print("CHARLIE TRAIN 6")
    state_dict = trainer.model.state_dict()
    if trainer.args.should_save:
        print("CHARLIE TRAIN 7")
        cpu_state_dict = {
            key: value.cpu()
            for key, value in state_dict.items()
        }
        print("CHARLIE TRAIN 8")
        del state_dict
        # trainer._save(output_dir, state_dict=cpu_state_dict)  # noqa
        print("CHARLIE TRAIN 9")
...
def train():
    ...
    # ==============================================================================================
    # MOE RELATED CODE -- mostly removed
    ...
    # TOKENIZER CODE -- REMOVED EVERY THING OTHER THAN STABLELM
    ...
    # LOADING IMAGE TOWER AND MM LAYER
    ...
    # REMOVE CODE FOR `if training_args.bits in [4, 8]`
    # Only train odd layers' MLP (i.e. the non-MoE ones)
    model.model.requires_grad_(False)
    model.lm_head.requires_grad_(False)
    for layer_id in range(24):
        if layer_id % 2 == 1:
            for n, p in model.model.layers[layer_id].mlp.named_parameters():
                p.requires_grad = True

    print("CHARLIE REQUIRE GRAD PARAMS: ")
    ...
    # TRAINING CODE BELOW
    print(f"CHARLIE training_args: {training_args}")
    ...
    print("CHARLIE TRAIN 1")
    trainer.save_state()
    print("CHARLIE TRAIN 2")
